...
namespace Rizumu.GameObjects
{
    class Note
    {
        ...
        public Note(NoteMode mode, int screenWidth, int screenHeight)
        {
            bgw = screenWidth;
            bgh = screenHeight;
            ...
            if (mode == NoteMode.left)
            {
                ...
                NoteSprite = new Sprite(BaseX, BaseY, tex, Color.White);
        ...
        public void Draw(SpriteBatch spriteBatch, ref bool KeyPress, bool Paused, bool Ready, float rotation, ref int CurrentCombo, ref float visiondist, int Timer, bool Auto)
        {
            if (!Paused && Ready)
            {
